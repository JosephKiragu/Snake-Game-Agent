...
def create_dqn() -> nn.Module:
    """Create the DQN model with specified architecture"""
    return nn.Sequential(
        nn.Linear(16, 128),# 4 directions * 4 state variables
        nn.ReLU(),
        nn.Linear(128, 64),
        nn.ReLU(),
...
def store_experience(
    ...
    training_state['memory'].append(Experience(state, action, reward, next_state, done)
    )
...
def optimize_model(training_state:dict):
    ...
    # compute Q(s_t, a) - the model computes Q(s_t) , then we select the column of actions taken
